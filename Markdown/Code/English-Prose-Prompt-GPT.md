...
I have a technical LaTeX specification document that describes the implementation of a two-site DMRG algorithm using Matrix Product States and Matrix Product Operators. I need you to convert this document into informal English prose.

Rules you must follow exactly:

Preserve ALL technical content: every index convention, every tensor shape, every contraction sequence, every algorithmic step, every implementation constraint. Nothing may be omitted.
Remove all LaTeX formatting: no equation environments, no numbered equations, no \texttt{} markup, no tables, no section numbering.
Replace all mathematical notation with verbal descriptions. For example, instead of writing  write "the tensor T with three indices: b for the MPO bond, x for the bra bond, and y for the ket bond." Instead of writing an einsum string like 'bxy,ytY->btxY', describe the contraction in words: "contract T and M over the shared ket bond index y, producing a result with indices b, t, x, and Y."
Write in plain, continuous English paragraphs. You may use bullet points for lists of steps, but do not use any structured tables or formal notation.
Do NOT add any new information, explanations, or clarifications that are not in the original document. Do NOT remove any information. This must be a format conversion only, not a rewrite.
The output should read like a detailed but informal implementation guide written in conversational technical English.
Here is the LaTeX specification document to convert:
...
With array conventions Lold with indices b, x, y; A with indices x, s, a; and W with indices b, B, s, t, the validated NumPy implementation is the contraction of Lold, A, W, and the conjugate of A over shared indices b, x, y, s, and t, producing Lnew with indices B, a, c. In einsum form this is `bxy,xsa,bBst,ytc->Bac`.
...
Second, contract the site i MPO tensor with X. The intermediate Y has indices beta at bond i, alpha at bond i minus one, sigma-prime at site i, sigma at site i plus one, and alpha-prime at bond i plus one. It is obtained by summing over beta at bond i minus one and sigma at site i of W at site i with indices beta left, beta right, sigma at site i, and sigma-prime at site i, times X with beta left, alpha left, sigma at site i, sigma at site i plus one, and right bond alpha-prime.
...
Add the following sentence to proper position: "Form the two-site center tensor Theta with indices alpha, sigma, sigma-prime, beta by contracting M at site i (with indices alpha, sigma, gamma) with M at site i+1 (with indices gamma, sigma-prime, beta) over the shared bond index gamma."
